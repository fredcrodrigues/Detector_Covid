...
path = r'/home/fredson/project_covid/dataset/'
...
m_vgg = vgg16.VGG16( weights='imagenet', include_top=False, input_shape=(224,224,3))

vggModel = m_vgg.output
vggModel = AveragePooling2D(pool_size=(4, 4))(vggModel)
vggModel = Flatten(name="flatten")(vggModel)
vggModel = Dense(64, activation="relu")(vggModel)
vggModel = Dropout(0.5)(vggModel)
vggModel = Dense(2, activation="softmax")(vggModel)

model = Model(inputs = m_vgg.input, outputs= vggModel)
...
h_train = model.fit(a_data.flow(trainX, trainY, batch_size=5), validation_data=(testX,testY), epochs=30, callbacks=[es,mc], verbose=1)

model.save('models/weights/c_detect.h5')
